...
public class ReviewDAO {
	...
	public List<Review> selectReviewsFromUserToOwner(long userId) {
		...
		try {
			...
			while (rs.next()) {
				...
				result.add(
						new Review(name, date, comment, url, userUrl, propertyId, reviewerId, userName, city, country));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public List<Review> selectReviewsOfPropertyById(long propertyId) {
		...
		try {
			...
			while (rs.next()) {
				...
				result.add(new Review(reviewerId, userName, userUrl, comment, date, cleanliness, communication, checkin,
	...
	public void insertNewReviewFromUser(long bookingId, long propertyId, long userId, double r1, double r2, double r3,
			double r4, double r5, double r6, String comment) {
		Connection connection = Connector.makeConnection();
		PreparedStatement ps = null;
		try {
