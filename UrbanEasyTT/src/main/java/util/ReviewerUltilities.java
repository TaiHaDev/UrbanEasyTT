...
public class ReviewerUltilities {
	...
	public static List<Double> averageRating(List<Review> reviewsList) {
		List<Double[]> groupedRatings = reviewsList.stream().map(elem -> new Double[] {elem.getCleanliness(), elem.getCommunication(), elem.getCheckin(), elem.getAccuracy(), elem.getLocation(), elem.getValue()}).collect(Collectors.toList());
